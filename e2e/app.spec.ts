...
// Coverage of app.js is recorded for every page and asserted 100% by
// coverage.spec.ts. See server.ts for the page variants used to reach
// defensive branches the real content never triggers.
...
let TOTAL: number;
let FULL_COUNT: string;
let EDGES: [string, string, number][];
let CAT: string;
let ISOLATED_ID: string | undefined;
...
test.beforeAll(async ({ server }) => {
  ...
  const cats = [...html.matchAll(/<button[^>]*class=chip[^>]*>/g)]
    ...
    .map((dm) => dm?.[1] ?? dm?.[2])
    .filter((c): c is string => c !== undefined && c !== "All");
  FULL_COUNT = `${TOTAL} entries · ${cats.length} categories`;
  const blob =
    html.match(/id=edgeData[^>]*>(.*?)<\/script>/s) ?? html.match(/id="edgeData"[^>]*>(.*?)<\/script>/s);
...
const visibleCards = (page: Page) =>
  page.evaluate(
    () => [...document.querySelectorAll<HTMLElement>("#main article.card")].filter((a) => !a.hidden).length
  );
const countLine = (page: Page) => page.locator("#countLine").textContent();
...
test.describe("category chips", () => {
  test("filter to one category and back to All", async ({ page, server }) => {
    await page.goto(server + "/");
    const chip = page.locator(`#chips .chip[data-cat="${CAT}"]`);
    await chip.click();
...
test.describe("view toggle and chrono", () => {
  ...
  test("rapid chrono→list flip cancels the pending frame; resize redraws only chrono", async ({ page, server }) => {
    await page.goto(server + "/");
    await page.evaluate(() => {
      document.querySelector<HTMLElement>('#viewSeg button[data-v="chrono"]')!.click();
      document.querySelector<HTMLElement>('#viewSeg button[data-v="list"]')!.click();
    });
...
test.describe("cross-reference navigation", () => {
  test("xref click navigates, pulses, and resets filters only when needed", async ({ page, server }) => {
    ...
    const found = await page.evaluate(() => {
      const result: {
        cross?: { from: string; to: string; id: string };
        local?: { from: string; id: string };
      } = {};
      for (const a of document.querySelectorAll<HTMLElement>("#main .lineage a.xref")) {
        const from = (a.closest("article.card") as HTMLElement).dataset.cat!;
...
test.describe("appearance", () => {
  const meta = (page: Page) =>
    page.evaluate(() => (document.getElementById("metaTheme") as HTMLMetaElement).content);
  ...
  test("menu toggling, selection, and outside-pointer close", async ({ page, server }) => {
    ...
    expect(await meta(page)).toBe("#000000");
    await page.click("#themeBtn");
    await page.click('#themeMenu button[data-t="light"]');
    expect(await page.evaluate(() => document.documentElement.dataset.theme)).toBe("light");
    expect(await meta(page)).toBe("#f5f5f7");
    await page.click("#themeBtn");
  ...
  test("auto mode follows the OS scheme; explicit modes do not", async ({ page, server }) => {
    ...
    await expect.poll(() => meta(page)).toBe("#000000");
    await page.emulateMedia({ colorScheme: "light" });
    await expect.poll(() => meta(page)).toBe("#f5f5f7");
    await page.click("#themeBtn");
    await page.click('#themeMenu button[data-t="dark"]');
