...
| `e2e/` | Playwright browser tests (TypeScript, run with [Bun](https://bun.com)) for `templates/app.js` against the built page, with **100% line/branch/function coverage enforced** (the fixture server swaps the minified inline script for the pristine `app.js` so V8 coverage maps 1:1). |
...
bun install && bun run e2e     # browser tests for app.js (TypeScript; enforces
                               # 100% coverage; CHROMIUM_PATH=... to reuse a
                               # pre-installed Chromium)
bun run typecheck              # tsc --noEmit over the e2e suite
```
